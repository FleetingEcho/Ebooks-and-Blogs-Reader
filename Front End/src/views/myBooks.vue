<template>
  <div class="container">
    ...
    <div style="margin-top:50px;width:400px;text-align:center">
      <h3 style="margin-left:50%;margin-right:50%;width:400px;">
        <div>Search My Ebook</div>
      </h3>
      <Input
        :value="searchValue"
        @on-search="clickToSearch"
        ...
        <ul class="gallery holder gallery-3col">
          <template v-for="item in response[1]">
            ...
            >
              <a
                href="#"
                @click="jumpToBook(item.key)"
                class="gallery-thumb gallery-3-col"
                data-rel="prettyPhoto"
              >
                <img
                  v-if="Math.floor(Math.random() * 100) > 50"
                  :src="imgSrc"
                ...
                />
                <span class="date"
                  ><span>{{ item.updatedAt.substr(8, 2) }}</span>
              ...
              <a href="#" @click="jumpToBook(item.key)" class="project-details">
                <span class="project-title">{{ item.file_name }}</span
                ><br />
...
export default {
  ...
  methods: {
    ...
    pageChange(clickPage) {
      ...
      fetchEbooks(clickPage, searchVal)
        .then(res => {
          // console.log(res);
          this.response = res;
