        ...
        <Content class="content-con">
          <div>
            <Tabs type="card" @on-click="handleClickTab" :value="getTabNameByRoute($route)">
              <TabPane animated:true :label="labelRender(item)" :name="getTabNameByRoute(item)" v-for="(item, index) in tabList" :key="`tabNav${index}`"></TabPane>
            </Tabs>
          </div>
          <div class="view-box">
  ...
  watch:{
   ...
   methods: {
    ...
    handleCollapsed () {
      ...
      // console.log('tabList内容是'+this.tabList);
    },
    handleClickTab (id) {
    ...
    handleTabRemove (id, event) {
      ...
      this.handleRemove({
        ...
        if (nextRoute.name!==undefined) this.$router.push(nextRoute)
